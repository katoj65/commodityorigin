...
function submit() {
    form.post(route('farm.soil-profiles.store', props.farmId), {
        preserveScroll: true,
        onSuccess: () => {
            closeDialog();
            ElNotification({ title: 'Soil Profile Recorded', message: 'The soil profile entry was added to this farm.', type: 'success', duration: 3200, offset: 84 });
        },
    });
}
</script>
    ...
    >
        <template #header>
            <div class="afsp-modal__head">
                ...
                <div class="afsp-modal__head-text">
                    ...
                    <div class="afsp-modal__title">Add Soil Profile</div>
                </div>
        ...
        <div class="afsp-modal__body">
            <div class="afsp-field">
                <label class="afsp-field__label">Soil Type</label>
                <el-select v-model="form.item" placeholder="Select a soil type" filterable class="afsp-input w-100" :class="{ 'afsp-input--error': form.errors.item }">
                    <el-option v-for="option in itemOptions" :key="option.slug" :label="option.name" :value="option.slug" />
                </el-select>
                <span v-if="form.errors.item" class="afsp-field__error">{{ form.errors.item }}</span>
            </div>

            <div class="afsp-field">
                <label class="afsp-field__label">Description <small>(optional)</small></label>
                <el-input v-model="form.description" type="textarea" :rows="3" placeholder="Add any detail worth recording — pH, texture, drainage, when it was assessed, etc." class="afsp-input" :class="{ 'afsp-input--error': form.errors.description }" />
        ...
        <template #footer>
            <div class="afsp-modal__footer">
                ...
                <button type="button" class="afsp-btn-primary" :disabled="form.processing" @click="submit">
                    {{ form.processing ? 'Saving…' : 'Add Soil Profile' }}
                </button>
            </div>
        </template>
    </el-dialog>
