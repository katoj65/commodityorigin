...
/* Fixed option sets, matching FarmCollectionService::unitOptions() /
   paymentStatusOptions() exactly — same literals as AddFarmCollectionModal. */
...
watch(() => props.modelValue, (open) => {
    ...
    form.variety = props.collection.variety || '';
    form.harvest_season = props.collection.harvest_season || '';
    form.quantity = props.collection.quantity ?? '';
    form.unit = props.collection.unit || 'kg';
    form.initial_moisture = props.collection.initial_moisture ?? '';
    form.initial_defects = props.collection.initial_defects ?? '';
    form.initial_grade = props.collection.initial_grade || '';
    form.initial_quality_score = props.collection.initial_quality_score ?? '';
    form.collection_price = props.collection.collection_price ?? '';
    form.currency = props.collection.currency || 'USD';
...
function submit() {
    form.patch(route('farm.collections.update', [props.collection.farm_id, props.collection.id]), {
        preserveScroll: true,
        onSuccess: () => {
            closeDialog();
            ElNotification({ title: 'Collection Updated', message: 'The farm collection was saved.', type: 'success', duration: 3200, offset: 84 });
        },
    });
}
</script>
        ...
        <template #header>
            <div class="afc-modal__head">
                ...
                <div class="afc-modal__head-text">
                    ...
                    <div class="afc-modal__title">Edit Farm Collection</div>
                </div>
